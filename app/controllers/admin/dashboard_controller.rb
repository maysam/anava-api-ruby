...
module Admin
  # Server-rendered admin dashboard, mounted at /admin — a native Rails/ERB
  # port of the anava-web React reference app (recordings browser +
  # analytics). Reads through the same Recording model and RecordingAnalytics
  # service the JSON API uses.
  #
  # It used to be the site root; `/` now serves the public marketing website
  # out of public/index.html, so this operator-facing view moved under /admin
  # behind Admin::BaseController's optional HTTP Basic auth. It is GET-only and
  # stateless — the per-user panel (PanelController) is the one HTML endpoint
